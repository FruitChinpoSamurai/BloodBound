...
import RequestsCreatedList from '../utilities/RequestsCreatedList';

export default function RequestsCreatedScreen(props) {
	const { posts } = useSelector((state) => state.user);
	const Posts = useSelector((state) => posts.map(id => state.posts[id]).filter(post => post != null));

	const dispatch = useDispatch();

	React.useEffect(() => dispatch(getPostsById(posts)), []);
